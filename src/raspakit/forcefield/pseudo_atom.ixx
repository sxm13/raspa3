...
{
  PseudoAtom() {};
  PseudoAtom(std::string name, double mass, double charge, double polarizability, size_t atomicNumber, bool printToPDB,
             std::string source = "")
      : name(name), mass(mass), charge(charge), polarizability(polarizability),
        atomicNumber(atomicNumber), printToPDB(printToPDB), source(source) {};
