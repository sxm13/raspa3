...
{
  PseudoAtom() {};
  PseudoAtom(std::string name, double mass, double charge, double polarizibility, size_t atomicNumber, bool printToPDB,
             std::string source = "")
      : name(name), mass(mass), charge(charge), polarizibility(polarizibility), atomicNumber(atomicNumber), printToPDB(printToPDB), source(source) {};
  uint64_t versionNumber{1};
  std::string name{"C"};
  double mass{1.0};
  double charge{0.0};
  double polarizibility{0.0};
