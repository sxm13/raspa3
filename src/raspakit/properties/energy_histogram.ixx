...
export struct PropertyEnergyHistogram
{
  ...
  PropertyEnergyHistogram(size_t numberOfBlocks, size_t numberOfBins, std::pair<double, double> range, size_t sampleEvery, size_t writeEvery)
      : numberOfBlocks(numberOfBlocks), 
        numberOfBins(numberOfBins),
        range(range),
        sampleEvery(sampleEvery),
        writeEvery(writeEvery),
        bookKeepingEnergyHistogram(std::vector<std::vector<double4>>(numberOfBlocks, std::vector<double4>(numberOfBins))),
