...
void PropertyNumberOfMoleculesHistogram::addSample(size_t blockIndex, size_t currentCycle, std::vector<size_t> numberOfIntegerMoleculesPerComponent, const double &weight)
{
  size_t bin;
...
std::vector<std::vector<double>> PropertyNumberOfMoleculesHistogram::averagedProbabilityHistogram() const
{
  ...
  for (size_t blockIndex = 0; blockIndex != numberOfBlocks; ++blockIndex)
  {
    std::transform(summedBlocks.begin(), summedBlocks.end(),
                   ...
                   summedBlocks.begin(), [](const std::vector<double> &a, const std::vector<double> &b) { return a + b; });
  }
...
void PropertyNumberOfMoleculesHistogram::writeOutput(size_t systemId, std::vector<Component> &components, size_t currentCycle)
{
