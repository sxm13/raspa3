              ...
              std::vector<Molecule> &moleculePositions)
{
  ...
  if(numberOfBlocksMSD > maxNumberOfBlocksMSD)
  {
    ...
    msdSelfCount.resize(numberOfBlocksMSD, std::vector<std::vector<size_t>>(numberOfComponents, std::vector<size_t>(numberOfBlockElementsMSD, 0uz)));
    blockDataMSDSelf.resize(numberOfBlocksMSD, std::vector<std::vector<double3>>(numberOfParticles, std::vector<double3>(numberOfBlockElementsMSD, double3())));
    msdSelf.resize(numberOfBlocksMSD, std::vector<std::vector<double4>>(numberOfComponents, std::vector<double4>(numberOfBlockElementsMSD, double4())));

    msdOnsagerCount.resize(numberOfBlocksMSD, std::vector<std::vector<size_t>>(numberOfComponents, std::vector<size_t>(numberOfBlockElementsMSD, 0uz)));
    blockDataMSDOnsager.resize(numberOfBlocksMSD, std::vector<std::vector<double3>>(numberOfComponents, std::vector<double3>(numberOfBlockElementsMSD, double3())));
    msdOnsager.resize(numberOfBlocksMSD, std::vector<std::vector<std::vector<double4>>>(numberOfComponents,
    ...
    if(countMSD % static_cast<size_t>(std::pow(numberOfBlockElementsMSD, currentBlock)) == 0)
    {
      ...
      for (size_t i = 0; i != components.size(); ++i)
      {
        ...
        for (size_t m = 0; m != numberOfMoleculesPerComponent[i]; ++m)
        {
          ...
          std::shift_right(begin(blockDataMSDSelf[currentBlock][molecule_index]), end(blockDataMSDSelf[currentBlock][molecule_index]), 1);
      ...
      for(size_t k = 0; k < currentBlocklength; k++)
      {
        for (size_t i = 0; i != components.size(); ++i)
        {
          ...
          {
            double msd_x = (blockDataMSDOnsager[currentBlock][i][k].x - value_onsager[i].x) * (blockDataMSDOnsager[currentBlock][j][k].x - value_onsager[j].x);
            double msd_y = (blockDataMSDOnsager[currentBlock][i][k].y - value_onsager[i].y) * (blockDataMSDOnsager[currentBlock][j][k].y - value_onsager[j].y);
            double msd_z = (blockDataMSDOnsager[currentBlock][i][k].z - value_onsager[i].z) * (blockDataMSDOnsager[currentBlock][j][k].z - value_onsager[j].z);
    ...
    for(size_t currentBlock = 0; currentBlock < numberOfBlocksMSD; ++currentBlock)
    {
      ...
      for(size_t k = 1; k < currentBlocklength; ++k)
      {
        if(msdSelfCount[currentBlock][i][k] > 0)
        {
          ...
          stream_msd_self_output << std::format("{} {} {} {} {} (count: {})\n",
    ...
    for (size_t j = 0; j < components.size(); ++j)
    {
      ...
      for(size_t currentBlock = 0; currentBlock < numberOfBlocksMSD; ++currentBlock)
      {
        ...
        for(size_t k = 1; k < currentBlocklength; ++k)
        {
          ...
          {
            double fac = 1.0 / static_cast<double>(numberOfMoleculesPerComponent[i] * msdOnsagerCount[currentBlock][i][k]);

            stream_msd_collective_output << std::format("{} {} {} {} {} (count: {})\n",
                static_cast<double>(k) * dt,
                fac * msdOnsager[currentBlock][i][j][k].w,
                fac * msdOnsager[currentBlock][i][j][k].x,
                fac * msdOnsager[currentBlock][i][j][k].y,
                fac * msdOnsager[currentBlock][i][j][k].z,
                msdOnsagerCount[currentBlock][i][k]);
          }
        }
      }
